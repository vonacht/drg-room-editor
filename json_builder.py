...
def generate_entrance(de, entrance, num, ffill_num, entrance_num):
    ...
    match entrance.entrance_type:
        ...
        case _:
            print(f"Unknown entrance type: {entrance.entrance_type}, defaulting to Exit")
            entrance_type = 'ECaveEntranceType::Exit'
...
def generate_room(dr, drr, tags, bounds, num_fflines, num_entrances, name):
    ...
    new_room["CreateBeforeSerializationDependencies"] = [x for x in range(1, outer_index)] + [-13]
    return new_room
...
def build_json_and_uasset(room_json: dict):
    ...
    floodfill_list = [generate_floodfill(default_floodfillline, default_roomlinepoint, ffill, ii, len(FLOODFILLLINES), len(ENTRANCES)) for ii, ffill in enumerate(FLOODFILLLINES)]
    entrance_list = [generate_entrance(default_entrance, e, ii, len(FLOODFILLLINES), len(ENTRANCES)) for ii, e in enumerate(ENTRANCES)]
    room = generate_room(default_room, default_room_reference, TAGS, BOUNDS, len(FLOODFILLLINES), len(ENTRANCES), ROOM_NAME)
